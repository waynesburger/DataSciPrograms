...
"""
createSelectable repeatedly selects four x,y pairs and fills the selectable list
    with the two best fit pairs
Returns a filled list
"""
def createSelectable(pairs):
    #this empty array should contain the fittest options
    selectable = []
    for i in range(0, len(pairs)):
        # grab four contestants from the pairs list
        contestants = []
        for j in range(0,4):
            ind = rand.randrange(0, len(pairs))
            contestants.append(pairs[ind])
        
        # we only want the two highest fitnesses
        # so sort them and grab the first two
        contestants = sorted(contestants, key=lambda l:l['result'], reverse=True)
        
        selectable.append(contestants[0])
        selectable.append(contestants[1])
...
def evolutionCycle(pairs):
    ...
    selectable = createSelectable(pairs)
...
panda_pairs.to_csv(r'D:\\CS457\\Wayne_Project_3\\tournament_results_small.csv', header=True, index=False, sep=',', mode='a')
